...
Code derived from https://github.com/kriswiner/MPU6050
https://os.mbed.com/users/onehorse/code/MPU6050IMU/
...
namespace MPU6050_DRIVER { 
    ...
    #define ADO 1       // SET TO 1 for MSB board Plymouth Uni!!!!
    #if ADO
    ...
    // Set initial input parameters
        ...
        //Set up I2C, (SDA,SCL)
        I2C i2c;// SET FOR PB_11, PB_!) for Plymouth Uni MSB I2C2 SDA and SCL (I2C_SDA, I2C_SCL);
    ...
    public:
        MPU6050(PinName SDA, PinName SDC, int i2cFreq) : i2c(SDA, SDC)
        {
            //Configure I2C Interface
            i2c.frequency(i2cFreq);

            //Check device ID
            _whoami = readByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050);  // Read WHO_AM_I register for MPU-6050
            if (_whoami != 0x68) {
                DEBUG_PRINT("ERROR: MPU6050 Constructor: Wrong whoami value\n");
            }
            
            // Start by performing self test and reporting values
            MPU6050SelfTest(SelfTest); 

            DEBUG_PRINT("x-axis self test: acceleration trim within : "); DEBUG_PRINT("%f", SelfTest[0]); DEBUG_PRINT("%% of factory value \n\r");
            DEBUG_PRINT("y-axis self test: acceleration trim within : "); DEBUG_PRINT("%f", SelfTest[1]); DEBUG_PRINT("%% of factory value \n\r");
            DEBUG_PRINT("z-axis self test: acceleration trim within : "); DEBUG_PRINT("%f", SelfTest[2]); DEBUG_PRINT("%% of factory value \n\r");
            DEBUG_PRINT("x-axis self test: gyration trim within : ");     DEBUG_PRINT("%f", SelfTest[3]); DEBUG_PRINT("%% of factory value \n\r");
            DEBUG_PRINT("y-axis self test: gyration trim within : ");     DEBUG_PRINT("%f", SelfTest[4]); DEBUG_PRINT("%% of factory value \n\r");
            DEBUG_PRINT("z-axis self test: gyration trim within : ");     DEBUG_PRINT("%f", SelfTest[5]); DEBUG_PRINT("%% of factory value \n\r");  
            ...
            if(SelfTest[0] < 1.0f && SelfTest[1] < 1.0f && SelfTest[2] < 1.0f && SelfTest[3] < 1.0f && SelfTest[4] < 1.0f && SelfTest[5] < 1.0f) 
            {
                ...
                DEBUG_PRINT("MPU6050 FAILED SELF TEST\n");
            }
        }
    protected:
        ...
        void LowPowerAccelOnly()
        {
            ...
            wait_us(100000);  // Add delay for accumulation of samples
